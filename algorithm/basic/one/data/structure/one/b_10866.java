...
public class b_10866 {
  ...
  public static class MyDeque {
    ...
    public void push_front(int X) {
      ...
      // head 가 있으면 새 노드가 헤드가 될 수 있게
      // 이중 연결작업 해주고 헤드를 새 노드로 교체한다.
      if (head != null) {
        newNode.next = head;
        head.prev = newNode;
      }
    ...
    public void push_back(int X) {
      Node newNode = new Node(X);
      // tail 이 있으면 새 노드가 tail 이 될 수 있게
      // 이중 연결작업 해주고 tail 을 새 노드로 교체한다.
      if (tail != null) {
        newNode.prev = tail;
        tail.next = newNode;
      }
