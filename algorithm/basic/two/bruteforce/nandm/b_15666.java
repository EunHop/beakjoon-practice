...
public class b_15666 {
  ...
  public static void main(String[] args) throws IOException {
    ...
    permutation(distinctNums, 0, 0);
    System.out.println(sb.toString());
  }

  static void permutation(int[] distinctNums, int at, int depth) {
    // 재귀 함수 종료 조건
    if (depth == M) {
      for (int i = 0; i < M; i++) {
    ...
    for (int i = at; i < distinctNums.length; i++) {
      result[depth] = distinctNums[i];
      permutation(distinctNums, i, depth + 1);
    }
  }
}
